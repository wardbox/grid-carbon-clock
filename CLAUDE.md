...
and TypeScript support. It uses Wasp ^0.23.x as the full-stack framework.
...
- **Zod**: Must be `^4.x` — Wasp 0.23 uses `z.url()` which is Zod v4 syntax.
- **openai** has an optional peer dep on Zod v3 which conflicts; `.npmrc` has
  `legacy-peer-deps=true` to resolve this.
- **lodash**: Use per-method imports (`import debounce from 'lodash/debounce'`)
  not named imports (`import { debounce } from 'lodash'`) — lodash is CJS and
  named imports break with Vite's ESM.

### Generated Code

- **Never edit `.wasp/out/`** — it is fully regenerated by Wasp on every run.
  Fix issues at the source in `main.wasp` or `src/`.

### main.wasp `head` array

- Only include valid self-closing HTML tags (e.g. `<meta ... />`).
- HTML comments (`<!-- -->`) are not valid JSX and will cause a build error.
  Remove them entirely rather than converting to JSX comments.
